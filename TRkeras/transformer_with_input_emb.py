...
class Transformer_i:
    ...
    def compile(self, optimizer='adam', active_layers=999):
        ...
        src_emb0 = Input(shape=(16, 256), dtype='float32')
        tgt_emb0 = Input(shape=(16, 256), dtype='float32')
        src_pos_emb0 = Input(shape=(16, 256), dtype='float32')
        tgt_pos_emb0 = Input(shape=(16, 256), dtype='float32')

        src_seq = src_seq_input
        tgt_seq = Lambda(lambda x: x[:, :-1])(tgt_seq_input)
        tgt_true = Lambda(lambda x: x[:, 1:])(tgt_seq_input)

        src_emb = add_layer([src_emb0, src_pos_emb0])#两个embedding相加
        tgt_emb = add_layer([tgt_emb0, tgt_pos_emb0])
        ...
        for i in range(16):
            ...
            self.permodel.append(Model([src_emb0,src_pos_emb0,tgt_emb0,tgt_pos_emb0], per_layer))
...
class LRSchedulerPerStep_i(Callback):
